...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!validate()) return;
    const result = await resetPassword(token, form.password, form.confirm);
    if (result.success) {
      setDone(true);
      setTimeout(() => router.push("/staff-portal/login"), 3000);
    }
  };
      ...
      <div className="max-w-md w-full">
        ...
        {done ? (
          ...
          >
            ...
            <div>
              <h3 className="font-semibold text-gray-900 text-lg">Password changed!</h3>
              <p className="text-sm text-gray-500 mt-2">Redirecting you to login…</p>
            </div>
          </motion.div>
        ) : (
          <>
            <h1 className="text-3xl font-bold text-[#082c2c] mb-2">New Password</h1>
            <p className="text-sm text-gray-500 mb-8">Create a strong password for your account.</p>
            ...
            <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-7">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                >
                  {isLoading ? <><SpinnerInline size={16} color="#fff" /> Resetting…</> : "Reset Password"}
                </button>
